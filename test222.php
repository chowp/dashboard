...
<?php
	...
	$select = "select count(*) as num ,smac from ing where  3 > (SELECT count( * ) FROM ing WHERE btime = a.btime AND  inf> a.inf ) and monitor_ap = '$monitor_ap' group by smac order by num desc";
	...
	$select = "SELECT btime,inf FROM ing a WHERE 3 > (SELECT count( * ) FROM ing WHERE btime = a.btime AND  inf> a.inf ) and smac = '$str1' and monitor_ap = '$monitor_ap'" ;
	$result = mysql_query($select);
	$row = array();
