...
<?php
	...
	$select = "select count(*) as num ,smac from ing a where  3 > (SELECT count( * ) FROM ing  WHERE btime = a.btime AND  inf> a.inf ) and monitor_ap = '$monitor_ap' and btime between '$starttime' and '$endtime' group by smac order by num desc ";
	...
	$select = "SELECT btime,inf,smac FROM ing a WHERE 3 > (SELECT count( * ) FROM ing WHERE btime = a.btime AND  inf> a.inf ) and smac = '$str1' and monitor_ap = '$monitor_ap' and btime between '$starttime' and '$endtime' order by btime" ;
	$result = mysql_query($select);
	$row = array();
	$arr3 = array();
